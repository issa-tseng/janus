...
(function() {
  var DomView, List, Varying, View, attr, definition, dynamic, extend, extendNew, isFunction, isString, match, varying, _ref, _ref1,
  ...
  _ref = require('../core/from')["default"], dynamic = _ref.dynamic, attr = _ref.attr, definition = _ref.definition, varying = _ref.varying;
  ...
  DomView = (function(_super) {
    ...
    DomView.prototype._render = function() {
      ...
      this._bindings = found(function(x) {
        return _this.constructor._point(x, _this.subject);
      });
      return dom;
    };

    DomView._point = match(dynamic(function(x, subject) {
      if (isFunction(x)) {
        return Varying.ly(x(subject));
      } else if (isString(x)) {
        return subject.watch(x);
      } else {
        return Varying.ly(x);
      }
    }), attr(function(x, subject) {
      return subject.watch(x);
    }), definition(function(x, subject) {
      return new Varying(subject.attribute(x));
    }), varying(function(x, subject) {
      if (isFunction(x)) {
        return Varying.ly(x(subject));
      } else {
        return Varying.ly(x);
