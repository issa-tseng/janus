...
describe 'collection', ->
  describe 'catted list', ->
    ...
    it 'should update when an element is added to a component list', ->
      ...
      sl1.add(2.5)
      cl.length_.should.equal(7)
      for elem, idx in [ 1, 2, 2.5, 3, 4, 5, 6 ]
        cl.at_(idx).should.equal(elem)

      sl2.add(2.9, 0)
      cl.length_.should.equal(8)
      for elem, idx in [ 1, 2, 2.5, 2.9, 3, 4, 5, 6 ]
        cl.at_(idx).should.equal(elem)

      sl3.add(5.5, 1)
      cl.length_.should.equal(9)
    ...
    it 'should update when an element is removed from a component list', ->
      ...
      sl2.removeAt(0)
      cl.length_.should.equal(2)
      for elem, idx in [ 1, 3 ]
        cl.at_(idx).should.equal(elem)

      sl1.removeAt(0)
      cl.length_.should.equal(1)
      cl.at_(0).should.equal(3)
