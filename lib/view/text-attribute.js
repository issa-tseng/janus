...
(function() {
  var $, Base, DomView, MultilineTextAttributeEditView, Text, TextAttributeEditView, Varying, find, from, ref, template, textAttributeTemplate,
  ...
  textAttributeTemplate = function(type, handler) {
    return template(find('input').attr('type', from.self().map(function(view) {
      var ref1;
      return (ref1 = view.options.type) != null ? ref1 : type;
      ...
      return subject.watchValue().map(function(x) {
        return x != null ? x : '';
      });
    })).on('input change', handler));
  };

  TextAttributeEditView = DomView.build($('<input/>'), textAttributeTemplate('text', function(event, subject) {
