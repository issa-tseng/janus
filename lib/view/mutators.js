  ...
  mutators = {
    ...
    render: function(data, args) {
      ...
      result = function(dom, point) {
        var _vendView;
        _vendView = function(subject, context, app, find, options) {
          return app.vendView(subject, extendNew(find != null ? find : {}, {
            context: context,
            constructorOpts: options
          }));
        };
        return Varying.flatMapAll(_vendView, terminate(data).point(point), doPoint(args.context, point), doPoint(from.app(), point), doPoint(args.find, point), doPoint(args.options, point)).reactNow(function(view) {
          var ref1;
