...
describe 'collection', ->
  describe 'FlattenedSet', ->
    it 'should start with parent basic elements', ->
      ...
      for x in [ 1, 2, 3, 4 ]
        flattened.includes(x).should.equal(true)
    ...
    it 'should track parent primitive changes', ->
      ...
      flattened.includes(4).should.equal(true)
      ...
      flattened.includes(2).should.equal(false)
    ...
    it 'should track flattened subelements on change', ->
      ...
      og.add(nest1)
      flattened.length.should.equal(4)
      flattened.includes(3).should.equal(true)
      flattened.includes(4).should.equal(true)

      nest2 = new Set([ 1, 3, 5 ])
      og.add(nest2)
      flattened.length.should.equal(5)
      flattened.includes(5).should.equal(true)

      og.remove(nest1)
      flattened.length.should.equal(4)
      flattened.includes(4).should.equal(false)

      og.remove(nest2)
      flattened.length.should.equal(2)
      flattened.includes(5).should.equal(false)

      og.remove(1)
      flattened.length.should.equal(1)
      flattened.includes(1).should.equal(false)
    ...
    it 'should not flatten more than one level', ->
      ...
      flattened.length.should.equal(5)
      flattened.includes(inner).should.equal(true)
    ...
    it 'should never attempt to flatten its parent (on create)', ->
      ...
      flattened.length.should.equal(3)
      flattened.includes(basiliskparrot).should.equal(true)
    ...
    it 'should never attempt to flatten its parent (on dynamic add)', ->
      ...
      parrot.add(parrot)
      flattened.length.should.equal(3)
      flattened.includes(parrot).should.equal(true)
    ...
    it 'should never try to flatten itself', ->
      ...
      flattened.length.should.equal(3)
      flattened.includes(flattened).should.equal(true)
